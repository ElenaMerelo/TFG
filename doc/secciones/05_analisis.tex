...
En esta subsección revisaremos algunos de los conceptos básicos de teoría de la probabilidad para el 
caso discreto. 
...
\dots \}$ que representa todos los posibles resultados de un experimento.
...
Dadas dos variables aleatorias $X,Y$ definidas en el mismo espacio muestral, y dos valores $x$ de $X$ e $y$ de 
...
Además, dos variables aleatorias $X$ e $Y$ son {\em independientes} sí y solo sí 
Two random variables $X$ and $Y$ are {\em independent} if and only if $$P(X=x, Y=y) = P(X=x) \, P(Y=y),$$ 
...
condicionada}; diremos que $X$ e $Y$ son {\em independientes condicionadamente dada $Z$} sí y solo sí todo par 
